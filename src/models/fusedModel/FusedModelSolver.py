...
class FusedModelSolver(object):
    ...
    def validate(self, dataloader: DataLoader) -> Tuple[torch.tensor, dict, torch.tensor, torch.tensor]:
        soft_preds, golds = self.__forward_batch_plus(dataloader)
        soft_preds = soft_preds.detach().cpu()
        golds = golds.detach().cpu()
        mean_loss = self.criterion(soft_preds, golds)  # num_of_label should be 1 for it to work
        if self.n_gpu > 1:
            mean_loss = mean_loss.mean()  # mean() to average on multi-gpu.
        metrics_scores = self.get_scores(soft_preds, golds)
        return mean_loss, metrics_scores, soft_preds, golds
        ...
        with tqdm(total=len(self.train_dataloader), desc=f"Epoch {epoch_idx}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if batch_idx % steps_per_eval == 0:
                    ...
                    #     pin_memory=True)
                    mean_loss, metrics_scores, _, _ = self.validate(self.dev_dataloader)
    ...
    def batch_to_device(self, batch: Tuple):
        meta_data_l, subreddit_ids_l, input_ids_tlc_l, \
            attention_mask_tlc_l, input_ids_post_l, attention_mask_post_l, \
            meta_data_r, subreddit_ids_r, input_ids_tlc_r, attention_mask_tlc_r, \
            input_ids_post_r, attention_mask_post_r, labels = batch

        batch_input = {
            'meta_data_l': meta_data_l.to(self.device),
            'subreddit_ids_l': subreddit_ids_l.to(torch.int64).to(self.subreddit_embedding_device),
            'input_ids_tlc_l': input_ids_tlc_l.to(torch.int64).to(self.device),
            'attention_mask_tlc_l': attention_mask_tlc_l.to(torch.int64).to(self.device),
            'input_ids_post_l': input_ids_post_l.to(torch.int64).to(self.device),
            'attention_mask_post_l': attention_mask_post_l.to(torch.int64).to(self.device),
            'meta_data_r': meta_data_r.to(self.device),
            'subreddit_ids_r': subreddit_ids_r.to(torch.int64).to(self.subreddit_embedding_device),
            'input_ids_tlc_r': input_ids_tlc_r.to(torch.int64).to(self.device),
            'attention_mask_tlc_r': attention_mask_tlc_r.to(torch.int64).to(self.device),
            'input_ids_post_r': input_ids_post_r.to(torch.int64).to(self.device),
            'attention_mask_post_r': attention_mask_post_r.to(torch.int64).to(self.device),
    ...
    @staticmethod
    def get_scores(soft_preds, golds) -> dict:
        """
        ...
        return {'CrossEntropyLoss': loss_measure.forward(input=soft_preds, target=golds).item(),
                'accuracy': accuracy}
    ...
    def get_train_dataloader(cls, input_dir: Path, batch_size: int) -> DataLoader:
        encoded_data_path = input_dir / 'cached.train.annotation.albert.buffer'
    ...
    def get_dev_dataloader(cls, input_dir: Path, batch_size: int) -> DataLoader:
        encoded_data_path = input_dir / 'cached.dev.annotation.albert.buffer'
        dataset = cls.__set_dataset(encoded_data_path)
    ...
    def get_test_dataloader(cls, input_dir: Path, batch_size: int) -> DataLoader:
        encoded_data_path = input_dir / 'cached.test.annotation.albert.buffer'
        dataset = cls.__set_dataset(encoded_data_path)
    ...
    @staticmethod
    def get_fused_model_arguments():
        parser = argparse.ArgumentParser(description='Arguments for Eigenmetric Regression')
        # model parameters
        parser.add_argument('--pretrained_feature_extractor_name_or_path', type=str,
                            help='The path to pretrained feature extractor.')
        parser.add_argument('--freeze_feature_extractor', action='store_true',
                            help='whether to freeze albert models')
        # solver parameters
        ...
        parser.add_argument('--seed', type=int, default=42,
                            help='the randoms seed of the whole process')
