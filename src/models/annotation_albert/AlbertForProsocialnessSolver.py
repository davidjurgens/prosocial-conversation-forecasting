...
class AlbertForProsocialSolver(object):
    ...
    def __init__(self,
        ...
        logx.initialize(logdir=output_dir,
                        ...
                        tensorboard=True,
                        no_timestamp=False,
                        hparams={"solver_hparams": self.state_dict(), "model_hparams": "albert-base-v2"},
                        eager_flush=True)
        # arguments
        ...
        with tqdm(total=len(self.train_dataloader), desc=f"Epoch {epoch_idx}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if batch_idx % steps_per_eval == 0:
                    ...
                    if self.n_gpu > 1:
                        save_dict = {"model_construct_params_dict": "albert-base-v2",
    ...
    def batch_to_device(self, batch) -> Tuple[dict, torch.tensor]:
        ...
        batch_input = {
            'input_ids': input_ids_tlc.to(torch.int64).to(self.device),
            'attention_mask': attention_mask_tlc.to(torch.int64).to(self.device),
