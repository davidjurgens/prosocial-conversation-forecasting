...
class EigenmetricRegressionSolver(object):
    ...
    def __init__(self,
        ...
        self.subreddit_embedding_device = torch.device("cpu")
    ...
    def from_pretrained(cls,
        ...
        if resume_training:
            ...
            solver_args["learning_rate"] = kwargs.pop("learning_rate", solver_args["learning_rate"])
    ...
    def batch_to_device(self, batch) -> Tuple[dict, torch.tensor]:
        if len(batch) == 7:
            meta_data, subreddit_ids, input_ids_tlc, attention_mask_tlc, \
            ...
            meta_data, subreddit_ids, input_ids_tlc, attention_mask_tlc, \
                input_ids_post, attention_mask_post= batch
            batch_input = {
                'meta_data': meta_data.to(self.device),
                'subreddit_ids': subreddit_ids.to(torch.int64).to(self.subreddit_embedding_device),
                'input_ids_tlc': input_ids_tlc.to(torch.int64).to(self.device),
    ...
    def __forward_batch_plus(self, dataloader: DataLoader):
        ...
        soft_preds = torch.cat(preds_list, dim=0).cpu()
        golds = torch.cat(golds_list, dim=0).cpu() if has_labels else None
        return soft_preds, golds
    ...
    def infer(self, dataset=None, batch=None) -> torch.tensor:
        if batch and not dataset:
            soft_preds, _ = self.__forwarding_step(batch)
            return soft_preds
        # if a dataset is passed, ignore any batch passed
        if dataset and not batch:
            dataloader = DataLoader(dataset, shuffle=False, batch_size=self.batch_size)
            soft_preds, _ = self.__forward_batch_plus(dataloader)
            return soft_preds
        raise AssertionError("both dataset and batch are passed; or neither of them is passed")
    ...
    def get_eigenmetric_regression_arguments():
        ...
        # model parameters
        parser.add_argument('--top_comment_pretrained_model_name_or_path', type=str,
                            help='The path to the top comment model.')
        parser.add_argument('--post_pretrained_model_name_or_path', type=str, default='output',
                            help='The path to the post model.')
        parser.add_argument('--classifier_dropout_prob', type=float, default=0.5,
                            help='the dropout probability of the layer before the classifier')
        parser.add_argument('--subreddit_pretrained_path', type=str, default=None,
                            help='the path to the pretrained subreddit embeddings')
        parser.add_argument('--num_subreddit_embeddings', type=int, default=None,
                            help='the size of total subreddit embeddings')
        parser.add_argument('--subreddit_embeddings_size', type=int, default=None,
                            help='the dimension of the subreddit embeddings')
        ...
        parser.add_argument('--freeze_alberts', action='store_true',
